...
import { useIllustrations, useDeleteIllustration, type Illustration } from "../../hooks/illustrations/useIllustrations";
import { ConfirmModal } from "../ConfirmModal";

interface AdminIllustrationListProps {
  onEdit: (item: Illustration) => void;
}

export function AdminIllustrationList({ onEdit }: AdminIllustrationListProps) {
  const [page, setPage] = useState(1);
  const limit = 10;
  
  const { data, isLoading } = useIllustrations(page, limit);
  const deleteMutation = useDeleteIllustration();
    ...
    <div className="bg-bg-secondary border border-border-subtle rounded-2xl overflow-hidden shadow-sm animate-slide-up">
      ...
      {deleteTarget && (
        <ConfirmModal
          isOpen={true}
          title="Delete Item"
          message={`Are you sure you want to delete this illustration? This action cannot be undone.`}
          confirmText="Delete"
          onConfirm={() => {
